...
class Constructor:
    def __init__(self, radio_code: int = 0):
    ...
    def convert(self, key, value, command, command_number):
        if command == 'T':
            if key not in self.trap_commands:
                print(f'Error in  Trap set: {key}')
            return single_int(key, value, command, command_number)
        elif command == 'S':
            if key not in self.set_commands:
                print(f'Error in  Set set: {key}')
            return self.converter.get(key, single_int)(key, value, command, command_number)
        elif command == 'G':
            if key not in self.get_commands:
                print(f'Error in  Get set: {key}')
            if key in self.get_need_value:
                return single_int(key, value, command, command_number)
            else:
                return single_none(key, None, command, command_number)
