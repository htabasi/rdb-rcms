...
class QueryGenerator(Thread):
    def __init__(self, parent, executor, log):
        ...
        self.event_adjustment = EAdjustmentInserter(self.radio.radio, self.log)
        self.event_cbit = ECBITInserter(self.radio.radio, self.log)
        self.event_connection = EConnectionInserter(self.radio.radio, self.log)
        self.event_status = EStatusInserter(self.radio.radio, self.log)
        self.event_list = EventListInserter(self.radio.radio, self.log)
        self.event_network = ENetworkInserter(self.radio.radio, self.log)
        self.event_operation = EOperationInserter(self.radio.radio, self.log)
        self.event_session = ESessionInserter(self.radio.radio, self.log)
        self.event_set_command = ESetCommandInserter(self.radio.radio, self.log)

        self.setting_access = SAccessInserter(self.radio.radio, self.log)
        self.setting_cbit = SCBITInserter(self.radio.radio, self.log)
        self.setting_configuration = SConfigurationInserter(self.radio.radio, self.log)
        self.setting_installation = SInstallationInserter(self.radio.radio, self.log)
        self.setting_inventory = SInventoryInserter(self.radio.radio, self.log)
        self.setting_ip = SIPInserter(self.radio.radio, self.log)
        self.setting_network = SNetworkInserter(self.radio.radio, self.log)
        self.setting_snmp = SSNMPInserter(self.radio.radio, self.log)
        self.setting_software = SSoftwareInserter(self.radio.radio, self.log)
        self.setting_status = SStatusInserter(self.radio.radio, self.log)

        self.variation_temperature = VTemperatureInserter(self.radio.radio, self.log)
        self.variation_voltage = VVoltageInserter(self.radio.radio, self.log)

        if self.radio.radio.type == 'RX':
            self.event_trx_operation = ERXOperationInserter(self.radio.radio, self.log)
            self.setting_trx_configuration = SRXConfigurationInserter(self.radio.radio, self.log)
            self.variation_radio = VReceptionInserter(self.radio.radio, self.log)
        else:
            self.event_special = SSpecialSettingInserter(self.radio.radio, self.log)
            self.event_trx_operation = ETXOperationInserter(self.radio.radio, self.log)
            self.setting_trx_configuration = STXConfigurationInserter(self.radio.radio, self.log)
            self.variation_radio = VTransmissionInserter(self.radio.radio, self.log)
